...
contract Oracle is Ownable {
    ...
    function update(address tokenA, address tokenB) external returns (bool) {
        ...
        (uint256 price0Cumulative, uint256 price1Cumulative, ) = KswapOracleLibrary.currentCumulativePrices(pair);
    ...
    function getQuantity(address token, uint256 amount) public view returns (uint256 quantity) {
        uint256 decimal = IERC20Kswap(token).decimals();
    ...
    function getAveragePrice(address token) public view returns (uint256 price) {
        ...
        if (token == anchorToken) {
            price = amount;
        } else if (KswapLibrary.pairFor(factory, token, anchorToken) != address(0)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (
                    KswapLibrary.pairFor(factory, token, intermediate) != address(0) &&
                    KswapLibrary.pairFor(factory, intermediate, anchorToken) != address(0)
                ) {
    ...
    function getCurrentPrice(address token) public view returns (uint256 price) {
        uint256 anchorTokenDecimal = IERC20Kswap(anchorToken).decimals();
        uint256 tokenDecimal = IERC20Kswap(token).decimals();

        if (token == anchorToken) {
            price = 10**anchorTokenDecimal;
        } else if (KswapLibrary.pairFor(factory, token, anchorToken) != address(0)) {
            (uint256 reserve0, uint256 reserve1) = KswapLibrary.getReserves(factory, token, anchorToken);
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (
                    ...
                    (uint256 reserve0, uint256 reserve1) = KswapLibrary.getReserves(factory, token, intermediate);
                    uint256 amountOut = 10**tokenDecimal.mul(reserve1).div(reserve0);
                    (uint256 reserve2, uint256 reserve3) = KswapLibrary.getReserves(factory, intermediate, anchorToken);
    ...
    function getLpTokenValue(address _lpToken, uint256 _amount) public view returns (uint256 value) {
        uint256 totalSupply = IERC20Kswap(_lpToken).totalSupply();
        address token0 = IKswapPair(_lpToken).token0();
        address token1 = IKswapPair(_lpToken).token1();
        uint256 token0Decimal = IERC20Kswap(token0).decimals();
        uint256 token1Decimal = IERC20Kswap(token1).decimals();
        (uint256 reserve0, uint256 reserve1) = KswapLibrary.getReserves(factory, token0, token1);
