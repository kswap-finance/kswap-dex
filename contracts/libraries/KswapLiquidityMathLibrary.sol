...
library KswapLiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = KswapLibrary.getReserves(factory, tokenA, tokenB);

        require(reserveA > 0 && reserveB > 0, 'KswapArbitrageLibrary: ZERO_PAIR_RESERVES');
        ...
        if (aToB) {
            uint256 amountOut = KswapLibrary.getAmountOut(amountIn, reserveA, reserveB);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint256 amountOut = KswapLibrary.getAmountOut(amountIn, reserveB, reserveA);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = KswapLibrary.getReserves(factory, tokenA, tokenB);
        IKswapPair pair = IKswapPair(KswapLibrary.pairFor(factory, tokenA, tokenB));
        bool feeOn = IKswapFactory(factory).feeTo() != address(0);
