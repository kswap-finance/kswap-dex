...
contract TradingPool is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (minRet) {
            pool.accKstPerShare = pool.accKstPerShare.add(kstReward.mul(1e12).div(pool.quantity));
            pool.allocKstAmount = pool.allocKstAmount.add(kstReward);
            pool.accKstAmount = pool.accKstAmount.add(kstReward);
        }
        pool.lastRewardBlock = block.number;
    }
    ...
    function swap(
        ...
        address pair = KswapLibrary.pairFor(address(factory), input, output);
        ...
        if (user.quantity > 0) {
            uint256 pendingReward = user.quantity.mul(pool.accKstPerShare).div(1e12).sub(user.rewardDebt);
            if (pendingReward > 0) {
    ...
    function pendingKst(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (user.quantity > 0) {
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getKstBlockReward(pool.lastRewardBlock);
                uint256 kstReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accKstPerShare = accKstPerShare.add(kstReward.mul(1e12).div(pool.quantity));
                return user.pendingReward.add(user.quantity.mul(accKstPerShare).div(1e12).sub(user.rewardDebt));
            }
            if (block.number == pool.lastRewardBlock) {
                return user.pendingReward.add(user.quantity.mul(accKstPerShare).div(1e12).sub(user.rewardDebt));
            }
    ...
    function withdraw(uint256 _pid) public {
        ...
        updatePool(_pid);
        uint256 pendingAmount = pendingKst(_pid, tx.origin);
