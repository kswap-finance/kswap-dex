...
contract KswapRouter is IKswapRouter02, Ownable {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = KswapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = KswapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'KswapRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = KswapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'KswapRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidityWithPermit(
        ...
        IKswapPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransfer(token, to, IERC20Kswap(token).balanceOf(address(this)));
        IWOKT(WOKT).withdraw(amountETH);
        TransferHelper.safeTransferETH(to, amountETH);
    }
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? KswapLibrary.pairFor(factory, output, path[i + 2]) : _to;
            IKswapPair(KswapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'KswapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'KswapRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            KswapLibrary.pairFor(factory, path[0], path[1]),
    ...
    function swapExactETHForTokens(
        ...
        assert(IWOKT(WOKT).transfer(KswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = KswapLibrary.sortTokens(input, output);
            IKswapPair pair = IKswapPair(KswapLibrary.pairFor(factory, input, output));
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountInput = IERC20Kswap(input).balanceOf(address(pair)).sub(reserveInput);
                amountOutput = KswapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            if (tradingPool != address(0)) {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, KswapLibrary.pairFor(factory, path[0], path[1]), amountIn);
        uint256 balanceBefore = IERC20Kswap(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20Kswap(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWOKT(WOKT).transfer(KswapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20Kswap(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20Kswap(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, 'KswapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
