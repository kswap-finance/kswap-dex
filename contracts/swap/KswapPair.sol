...
contract KswapPair is KswapERC20 {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint256 denominator = rootK.mul(IKswapFactory(factory).feeToRate()).add(rootKLast);
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (data.length > 0) IKswapCallee(to).KswapCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20Kswap(_token0).balanceOf(address(this));
            balance1 = IERC20Kswap(_token1).balanceOf(address(this));
        }
    ...
    function skim(address to) external lock {
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
        _safeTransfer(_token0, to, IERC20Kswap(_token0).balanceOf(address(this)).sub(reserve0));
        _safeTransfer(_token1, to, IERC20Kswap(_token1).balanceOf(address(this)).sub(reserve1));
    }
